...
app.post('/v1/calculator', (req, res) => {
    try {
        let { propertyPrice, downPayment, interestRate, amortizationPeriod, paymentSchedule } = req.body;

        let numPaymentsPerYear = paymentSchedule.toUpperCase() === PaymentSchedule.BIWEEKLY ? 26 : 12;
        let principal = propertyPrice - (propertyPrice * (downPayment/100));
        let interestRatePerSchedule = interestRate/100/numPaymentsPerYear;
